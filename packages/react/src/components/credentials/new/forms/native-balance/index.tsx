...
import {
  NewNativeBalanceCredentialProvider,
  useNewNativeBalanceCredential,
} from './context'
import {
  formatAmount,
  FungiblePosition,
...
export function NativeBalanceCredentialForm({
  initialBalance,
  isOpen,
  setIsOpen,
  parentId,
  ...
  return (
    ...
    >
      <YStack gap="$2">
        ...
        <NativeBalanceSubmitButton />
      </YStack>
    </NewNativeBalanceCredentialProvider>
  )
}

function NativeBalanceWalletField() {
  const { connectWallet } = useNewNativeBalanceCredential()

  return <WalletField connectWallet={connectWallet} />
}

function TokenField() {
  const { tokenId, setTokenId, setBalance } = useNewNativeBalanceCredential()
  ...
  useEffect(() => {
    ...
    let selectedToken = fungibles.find((t) => {
      try {
        getZerionChain(t.relationships.chain.data.id)
        return true
      } catch (e) {
        return false
      }
    ...
    if (tokenId) {
      ...
      const foundToken = fungibles.find((t) => {
        ...
        const impl = t.attributes.fungible_info.implementations.find(
          (i) => i.address === null && i.chain_id === chain.zerionId
        )
        return impl
      })
      if (foundToken) {
    ...
    <XStack ai="center" jc="space-between" w="100%">
      <XStack gap="$3" ai="center">
        <Image
          width={28}
          height={28}
          src={token.attributes.fungible_info.icon?.url ?? ''}
        />
        <YStack>
          <Text fos="$2" fow="500">
      ...
      <XStack gap="$2" ai="center">
        <YStack ai="flex-end">
          <Text fos="$2" fow="500">
            {`${formatAmount(token.attributes.quantity.float)}`}
          </Text>
          <Text fos="$1" fow="400" color="$color11">
...
function NativeBalanceSubmitButton() {
  const { address } = useAccount()
